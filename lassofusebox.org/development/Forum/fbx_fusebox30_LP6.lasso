...
Library:(LassoApp_Link:'fbx_Library.lasso');
...
// FORM and URL variables to VARIABLES scope. Lasso requires sessions be started before 
// [Action_Params] are converted to variables. Therefore, we attempt to include the 
// fbx_Sessions.lasso file. This file is exclusive to the Lasso version of Fusebox.
// -------------------------------------------------------------------------------------------

$_section = 4;

Protect;
	Include:(LassoApp_Link:'fbx_Sessions.lasso');
/Protect;

Iterate:(Action_Params),(Variable:'_temp');
	If:!($_temp->First)->(BeginsWith:'-');
		Variable:($_temp->First) = Action_Param:($_temp->First);
	/If;
/Iterate;
...
Include:(LassoApp_Link:'fbx_Circuits.lasso');
...
Include:(LassoApp_Link:'fbx_Settings.lasso');
...
Iterate:$FB_->(Find:'fullPath')->(Split:'/'),(Variable:'aPath');
	...
	If:$_ReverseCircuits->(Find:$_Circuits ->(Find:$fusebox->(Find:'HomeCircuit')) + '/' + $FB_->(Find:'Corepath'));
		...
		Else;
			$fusebox->(Insert:'IsTargetCircuit' = false);
		/If;
		Include:(LassoApp_Link:$fusebox->(Find:'currentPath') + 'fbx_Settings.lasso');
	/If;
/Iterate;
...
FBX_SaveContent: -Variable='_layout';
	Include:(LassoApp_Link:$FB_->(Find:'fuseboxpath') + 'fbx_Switch.lasso');
/FBX_SaveContent;
...
While:$FB_->(Find:'layoutpath')->Size;
	If:$_ReverseCircuits->(Find:$FB_->(Find:'circuitalias'));
		...
		Protect;
			Include:(LassoApp_Link:$fusebox->(Find:'ThisLayoutPath') + 'fbx_Layouts.lasso');
			Handle:false;
		...
		If:$fusebox->(Find:'layoutfile')->Size;
			FBX_SaveContent: -Variable='_layout';
				Include:(LassoApp_Link:$fusebox->(Find:'ThisLayoutPath') + $fusebox->(Find:'layoutdir') + $fusebox->(Find:'layoutfile'));
			/FBX_SaveContent;
			$fusebox->(Insert:'layout' = @$_layout);
		/If;
	/If;
